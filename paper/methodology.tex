...
Recall from before the $p(X)$ to be the true, unknown propensity score. $\mu_{d,t}(X)$ is being a model for the true, unknown outcome regression $m_{d,t}(x)\equiv \mathbb{E}[Y_t | D = d, X = x]$, $d, t = 0, 1$.
...
\begin{equation}
\tau^{dr} = \mathbb{E} \left[ \left( w_1(D) - w_0(D, X; p) \right) \left( \Delta Y - \mu_{0, \Delta}(X) \right) \right],
\end{equation}

where, for a generic $g$,
%explain that more and check if it is correctly written to my notation
\begin{equation}
w_1(D) = \frac{D}{\mathbb{E}[D]},
\end{equation}

and

\begin{equation}
w_0(D, X; g) = \frac{g(X)(1-D)}{1 - g(X)} \Bigg/ \mathbb{E} \left[ \frac{g(X)(1-D)}{1 - g(X)} \right].
\end{equation}
