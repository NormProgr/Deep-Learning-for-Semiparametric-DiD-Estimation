...
\label{eq:6}
\end{equation}
The \ac{drdid} estimator is consistent and unbiased if both \ac{or} and \ac{ipw} are correctly specified but it is less clear if only one of the two is correctly specified.
...
Intuitively, because the \ac{or} correctly identifies that the change in the outcome of control should not change over time, as it is not treated, any multiplication of it becomes zero as well.
